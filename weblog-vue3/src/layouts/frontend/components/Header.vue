<template>
        ...
        <nav class="bg-white border-gray-200 border-b dark:bg-gray-900 dark:border-gray-800">
            <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                ...
                <div class="flex items-center md:order-2">
                    <!-- 白天黑夜切换 -->
                    <label for="switch" class="switch ml-4 mr-4">
                        <input id="switch" type="checkbox" :checked="!isDark" @change="toggleDark()" />
    ...
    <div id="update-modal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed  left-0 right-0 z-50 mx-auto w-full h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-2 w-full max-w-[90%] mx-auto">
            <div
                class="relative bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                <!-- Modal header 移动端标题优化 -->
                <div class="flex items-center justify-between p-3 md:p-4 border-b dark:border-gray-700">
                    <h3 class="text-base md:text-lg font-medium text-gray-800 dark:text-gray-200">
                        V1.1 版本更新公告&nbsp;5月3日
                    </h3>
                    <button type="button" @click="updateModal.hide()"
                ...
                <!-- Modal body 移动端内容优化 -->
                <div class="p-3 md:p-4 space-y-4 text-sm text-gray-600 dark:text-gray-400">
                    <!-- 核心功能 -->
                    <div class="space-y-2">
                        <div class="flex items-start gap-3">
                            <span class="text-gray-500 shrink-0 mt-0.5">🔍</span>
                            <div>
                                <h4 class="font-medium text-gray-800 dark:text-gray-200 mb-1 text-sm md:text-base">
                                    核心功能更新</h4>
                                <p class="leading-relaxed text-xs md:text-sm">
                                    【智能全域搜索】<br>
                                    新增多维度内容检索系统，支持标题/摘要/正文的全文跨库搜索
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- 视觉体验 -->
                    <div class="space-y-2 pt-2 border-t border-dashed dark:border-gray-700">
                        <div class="flex items-start gap-3">
                            <span class="text-gray-500 shrink-0 mt-0.5">🌓</span>
                            <div>
                                <h4 class="font-medium text-gray-800 dark:text-gray-200 mb-1 text-sm md:text-base">
                    ...
                    <div class="space-y-2 pt-2 border-t border-dashed dark:border-gray-700">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="leading-relaxed text-xs md:text-sm">
                                    【更细节的首页信息架构】<br>
                                    在用户信息/分类/标签卡片新增动态微交互效果，优化信息层级展示（新增访问量等数据）
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Modal footer 按钮优化 -->
                <div class="p-3 md:p-4 border-t dark:border-gray-700">
                    <button @click="confirmUpdate()" class="w-full px-4 py-2.5 text-sm font-medium text-white bg-gray-800 hover:bg-gray-700 rounded-lg 
                           transition-colors dark:bg-gray-700 dark:hover:bg-gray-600 active:scale-95">
                        浏览新版本
                    </button>
                </div>
            </div>
...
onMounted(() => {
    ...
    // 初始化版本更新模态框
    const $updateModalElement = document.querySelector('#update-modal');
    updateModal.value = new Modal($updateModalElement, modalOptions, instanceOptions);
...
const checkShowUpdateModal = () => {
    // 从本地存储中获取是否已确认过更新
    const hasConfirmedUpdate = localStorage.getItem('hasConfirmedUpdate');
...
const confirmUpdate = () => {
    ...
    // 将确认状态保存到本地存储
    localStorage.setItem('hasConfirmedUpdate', 'true');
}
